...
// =================== Shooting Star White Random Length ===================
...
//   uint8_t u = beat8(5, 0);            //BPM will allow you to adjust the speed the dot is moving.
//   int rand = random(0, NUM_LEDS - 1);
//   uint16_t pos_1 = map(u, 0, 255, 0, rand);
//   leds[pos_1] = CHSV(start_hue, 0, 255);
//   FastLED.show();
//   start_hue += delta_hue;
//   // if (pos_1 > NUM_LEDS / 2)
...
void wave_white()
{
  ...
  fadeToBlackBy(leds, NUM_LEDS, determine_length_shooting_star()); //change fadeby to smaller or larger number to adjust the length of the tail.
  uint8_t u = beat8(determine_speed_shooting_star(), 0);           //BPM will allow you to adjust the speed the dot is moving.
  uint16_t pos = map(u, 0, 255, 0, random(NUM_LEDS - 1));
  while (last_pos != pos)
  {
    if (last_pos < pos)
    {
